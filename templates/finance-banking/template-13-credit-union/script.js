/* Riverstone Credit Union — finance-banking interactions | dependency-free vanilla JS (file:// safe) */
(function () {
  "use strict";
  document.documentElement.classList.add("js");

  var BRAND = "Riverstone Credit Union";
  ...
  if (calcBox) {
    ...
    function recalc() {
      ...
      var amount = clampNum(get("calcAmount"), 100, 5000000, 25000);
      var rate = clampNum(get("calcRate"), 0, 40, 7.5);
      var years = clampNum(get("calcYears"), 1, 40, 5);
      setTxt(rangeOut("calcRate"), rate.toFixed(2) + "%");
      setTxt(rangeOut("calcYears"), years + (years === 1 ? " yr" : " yrs"));
      var n = years * 12;
      var r = rate / 100 / 12;
      var monthly = r > 0 ? amount * r / (1 - Math.pow(1 + r, -n)) : amount / n;
      var total = monthly * n;
      var interest = Math.max(total - amount, 0);
      setTxt(out1, money2(monthly));
      setTxt(sub, "per month for " + years + (years === 1 ? " year" : " years"));
      if (rowsWrap) {
        rowsWrap.innerHTML =
          row("Amount borrowed", money(amount)) +
          row("Total interest", money(interest)) +
          row("Total repaid", money(total));
      }
      if (bar && total > 0) { bar.style.width = Math.max(6, Math.round(amount / total * 100)) + "%"; }
