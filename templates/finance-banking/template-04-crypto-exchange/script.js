/* Voltcoin — finance-banking interactions | dependency-free vanilla JS (file:// safe) */
(function () {
  "use strict";
  ...
  if (calcBox) {
    ...
    function recalc() {
      ...
      var amount = clampNum(get("calcAmount"), 1, 10000000, 1000);
      var assetSel = document.getElementById("calcAsset");
      var rate = assetSel ? parseFloat(assetSel.value) : 1;
      if (!isFinite(rate) || rate <= 0) { rate = 1; }
      var label = assetSel ? (assetSel.options[assetSel.selectedIndex].getAttribute("data-label") || assetSel.value) : "units";
      var feePct = clampNum(get("calcFee"), 0, 5, 0.5);
      setTxt(rangeOut("calcFee"), feePct.toFixed(2) + "%");
      var fee = amount * feePct / 100;
      var net = amount - fee;
      var received = net / rate;
      setTxt(out1, fmt6(received) + " " + label);
      setTxt(sub, "you receive for " + money(amount));
      if (rowsWrap) {
        rowsWrap.innerHTML =
          row("You convert", money2(amount)) +
          row("Fee (" + feePct.toFixed(2) + "%)", money2(fee)) +
          row("Converted at", "1 " + label + " = " + money2(rate)) +
          row("You receive", fmt6(received) + " " + label);
      }
      if (bar && amount > 0) { bar.style.width = Math.max(6, Math.round(net / amount * 100)) + "%"; }
