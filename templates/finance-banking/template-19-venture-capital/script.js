/* Northwind Ventures — finance-banking interactions | dependency-free vanilla JS (file:// safe) */
(function () {
  "use strict";
  document.documentElement.classList.add("js");

  var BRAND = "Northwind Ventures";
  ...
  if (calcBox) {
    ...
    function recalc() {
      ...
      var qty = clampNum(get("calcQty"), 1, 1000000, 100);
      var price = clampNum(get("calcPrice"), 0.01, 1000000, 50);
      var feePct = clampNum(get("calcFee"), 0, 3, 0.1);
      setTxt(rangeOut("calcFee"), feePct.toFixed(2) + "%");
      var notional = qty * price;
      var commission = notional * feePct / 100;
      var total = notional + commission;
      setTxt(out1, money2(total));
      setTxt(sub, "estimated total order cost");
      if (rowsWrap) {
        rowsWrap.innerHTML =
          row("Order value", money2(notional)) +
          row("Fee / commission", money2(commission)) +
          row("Effective fee", (notional>0?(commission/notional*100):0).toFixed(2) + "%") +
          row("Total cost", money2(total));
      }
      if (bar && total > 0) { bar.style.width = Math.max(6, Math.round(notional / total * 100)) + "%"; }
