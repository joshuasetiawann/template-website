/* Truenorth Advisory — finance-banking interactions | dependency-free vanilla JS (file:// safe) */
(function () {
  "use strict";
  document.documentElement.classList.add("js");

  var BRAND = "Truenorth Advisory";
  ...
  if (calcBox) {
    ...
    function recalc() {
      ...
      var income = clampNum(get("calcIncome"), 0, 5000000, 65000);
      var statusSel = document.getElementById("calcStatus");
      var married = statusSel && statusSel.value === "married";
      var deduction = married ? 29200 : 14600;
      var taxable = Math.max(income - deduction, 0);
      // progressive brackets (single; doubled for married) — illustrative
      var brk = married
      ...
      for (var i = 0; i < brk.length; i++) {
        var lo = brk[i][0];
        var hi = (i + 1 < brk.length) ? brk[i + 1][0] : Infinity;
        if (taxable > lo) { tax += (Math.min(taxable, hi) - lo) * brk[i][1]; }
        else { break; }
      }
      var marginal = 0.10;
      for (var j = 0; j < brk.length; j++) { if (taxable > brk[j][0]) { marginal = brk[j][1]; } }
      var net = income - tax;
      var effRate = income > 0 ? (tax / income * 100) : 0;
      setTxt(out1, money(tax));
      setTxt(sub, "estimated federal tax · " + (married ? "married filing jointly" : "single filer"));
      if (rowsWrap) {
        rowsWrap.innerHTML =
          row("Taxable income", money(taxable)) +
          row("Marginal rate", (marginal * 100).toFixed(0) + "%") +
          row("Effective rate", effRate.toFixed(1) + "%") +
          row("Take-home (after fed tax)", money(net));
      }
      if (bar && income > 0) { bar.style.width = Math.max(4, Math.min(100, Math.round(effRate))) + "%"; }
