/* ============================================================
   The Riverside Get-Together — Community Meetup
   template-18-community-meetup  (variant V5)
...
(function () {
  ...
  function onHeaderScroll() {
    if (header) { header.classList.toggle('is-scrolled', window.scrollY > 9); }
  }
  ...
  if (toTop) {
    var onTopScroll = function () { toTop.classList.toggle('is-on', window.scrollY > 580); };
  ...
  if (countdownRoot) {
    var target = new Date('2026-07-11T11:00:00').getTime();
    var fields = {
    ...
    if (reduceMotion || !('IntersectionObserver' in window)) {
      ...
      var ro = new IntersectionObserver(function (entries, obs) {
        entries.forEach(function (entry) {
          ...
          if (idx < 0) { idx = 0; }
          setTimeout(function () { el.classList.add('is-in'); }, Math.min(idx, 6) * 100);
          obs.unobserve(el);
        });
  ...
  /* ---------- confetti sprinkle (decorative, reduced-motion aware) ---------- */
  var confettiHost = qs('[data-confetti]');
  if (confettiHost && !reduceMotion) {
    var colors = (confettiHost.getAttribute('data-confetti') || '').split(',');
    if (colors.length < 2) { colors = ['#F26B6B', '#8B5CF6', '#F6C453', '#22C55E']; }
    for (var i = 0; i < 18; i++) {
      ...
      dot.style.animationDuration = (3.4 + Math.random() * 3).toFixed(2) + 's';
  ...
  Array.prototype.forEach.call(btns, function (btn) {
    btn.addEventListener('click', function () {
      ...
      var done = function () {
        ...
        var live = btn.parentNode ? btn.parentNode.querySelector('.copy-live') : null;
