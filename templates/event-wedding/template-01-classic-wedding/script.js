/* ============================================================
   Eleanor & Theodore — Classic Wedding
   template-01-classic-wedding  (variant V1)
...
(function () {
  ...
  if (toTop) {
    var onTopScroll = function () { toTop.classList.toggle('is-on', window.scrollY > 560); };
  ...
  if (countdownRoot) {
    var target = new Date('2026-09-19T16:00:00').getTime();
    var fields = {
    ...
    if (reduceMotion || !('IntersectionObserver' in window)) {
      ...
      var ro = new IntersectionObserver(function (entries, obs) {
        entries.forEach(function (entry) {
          ...
          if (idx < 0) { idx = 0; }
          setTimeout(function () { el.classList.add('is-in'); }, Math.min(idx, 6) * 70);
          obs.unobserve(el);
        });
