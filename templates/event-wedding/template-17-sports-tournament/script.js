/* ============================================================
   Capital Cup 2026 — Sports Tournament
   template-17-sports-tournament  (variant V3)
...
(function () {
  ...
  if (toTop) {
    var onTopScroll = function () { toTop.classList.toggle('is-on', window.scrollY > 640); };
  ...
  if (countdownRoot) {
    var target = new Date('2026-08-08T09:00:00').getTime();
    var fields = {
    ...
    if (reduceMotion || !('IntersectionObserver' in window)) {
      ...
      var ro = new IntersectionObserver(function (entries, obs) {
        entries.forEach(function (entry) {
          ...
          if (idx < 0) { idx = 0; }
          setTimeout(function () { el.classList.add('is-in'); }, Math.min(idx, 6) * 90);
          obs.unobserve(el);
        });
  ...
  qsa('[data-tabs]').forEach(function (group) {
    ...
    var panels = qsa('[role="tabpanel"]', group);
  ...
  /* ---------- marquee (seamless, reduced-motion aware) ---------- */
  qsa('[data-marquee]').forEach(function (m) {
    var track = qs('.marquee-track', m);
    if (!track) { return; }
    if (reduceMotion) { m.classList.add('marquee-static'); return; }
  ...
  if (countEls.length && 'IntersectionObserver' in window && !reduceMotion) {
    var cObs = new IntersectionObserver(function (entries, obs) {
      entries.forEach(function (entry) {
        ...
        var step = function (ts) {
          ...
          el.textContent = Math.floor(p * to).toLocaleString() + suffix;
    ...
    countEls.forEach(function (el) {
      el.textContent = (parseInt(el.getAttribute('data-count'), 10) || 0).toLocaleString() + (el.getAttribute('data-suffix') || '');
    });
  }
