/* ============================================================
   Mara & Idris — Modern Wedding
   template-03-modern-wedding  (variant V2)
   Vanilla JS · no dependencies · file:// safe
   Features: mobile nav, live countdown, scrollspy, scroll-reveal,
             RSVP validation, back-to-top, footer year
   ============================================================ */
(function () {
  'use strict';
  ...
  if (toTop) {
    var onTopScroll = function () { toTop.classList.toggle('is-on', window.scrollY > 600); };
  ...
  if (countdownRoot) {
    var target = new Date('2026-11-07T17:00:00').getTime();
    var fields = {
    ...
    if (reduceMotion || !('IntersectionObserver' in window)) {
      ...
      var ro = new IntersectionObserver(function (entries, obs) {
        entries.forEach(function (entry) {
          ...
          if (idx < 0) { idx = 0; }
          setTimeout(function () { el.classList.add('is-in'); }, Math.min(idx, 6) * 60);
          obs.unobserve(el);
        });
...
(function () {
  'use strict';
  var triggers = document.querySelectorAll('[data-accordion] .acc-trigger');
  Array.prototype.forEach.call(triggers, function (btn) {
    btn.addEventListener('click', function () {
      var panel = document.getElementById(btn.getAttribute('aria-controls'));
      var open = btn.getAttribute('aria-expanded') === 'true';
      btn.setAttribute('aria-expanded', open ? 'false' : 'true');
      if (panel) { panel.hidden = open ? true : false; }
    });
  });
})();
