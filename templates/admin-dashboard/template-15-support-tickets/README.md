# Resolvo — Support Tickets Dashboard Template

A dense support operations dashboard template with SLA gauge, ticket volume combo chart, sparkline stats and a 10-row priority ticket queue.

Part of a 100-template library of pure HTML/CSS/JS website templates. This template uses the dense enterprise layout with a collapsible icon-rail sidebar.
...
- Combo chart: animated bars with an SVG line overlay drawn via stroke-dashoffset
- Goal gauge (SVG arc) animated with requestAnimationFrame
- Collapsible icon-rail sidebar (labels expand on toggle)
- JS-drawn sparklines inside every stat card
...
- Ticket volume — 14 days (combo chart)
- Priority queue — data table with 10 realistic rows, status pills and row actions
- Escalation log — timestamped feed
- SLA compliance — June (goal gauge)
...
- **Tickets** (`#/list`) — a searchable, paginated, CSV-exportable table of 20 tickets.
- **Support reports** (`#/reports`) — two extra charts, three insight tiles and key takeaways.
