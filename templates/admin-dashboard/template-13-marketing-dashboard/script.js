/* Funnelfox — marketing dashboard interactions (vanilla JS, no dependencies) */
(function () {
  'use strict';
  var doc = document;
  var body = doc.body;
  ...
  /* ----- Chart data (Funnelfox sample dataset) ----- */
  var SPEND = [{"label": "Jan", "values": [38, 92]}, {"label": "Feb", "values": [42, 101]}, {"label": "Mar", "values": [36, 88]}, {"label": "Apr", "values": [44, 118]}, {"label": "May", "values": [47, 132]}, {"label": "Jun", "values": [47, 141]}];
  renderBars('spendBars', SPEND, {"prefix": "$", "suffix": "k"});
  renderGauge('leadGauge', 0.76, function (p) { return Math.round(76 * p) + '%'; });


  /* ----- Funnelfox release upgrades: year, shimmer, reveal, pagination, export, tooltips ----- */
  /* ----- Reports view charts (rendered lazily on first show) ----- */
  window.__renderReports = function () {
    renderBars('repChartA', [{"label": "Search", "value": 42}, {"label": "Social", "value": 37}, {"label": "Partner", "value": 32}, {"label": "Display", "value": 18}, {"label": "Email", "value": 5}, {"label": "Video", "value": 14}], {"prefix": "$", "suffix": "k"});
    renderGauge('repChartB', 0.92, function (p) { return Math.round(92 * p) + '%'; });
  ...
  function applyRange(k) {
    renderBars('spendBars', k === 'b' ? SPEND.slice(3) : SPEND, {"prefix": "$", "suffix": "k"});
  }
