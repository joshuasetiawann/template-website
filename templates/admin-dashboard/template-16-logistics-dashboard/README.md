# Freightwise — Logistics Dashboard Template

A navy-and-amber logistics control tower dashboard template with dual-area volume chart, route network grid, tabbed shipments table and dark mode toggle.

Part of a 100-template library of pure HTML/CSS/JS website templates. This template uses the two-tone layout with a dark sidebar over light content.
...
- SVG line chart with gradient area fill, drawn in via a stroke-dashoffset animation
- Tab switching between two table row sets
...
- Network volume — 10 days (line chart)
- Shipment board — data table with 7 realistic rows, status pills and row actions
- Dispatch log — timestamped feed
- Route network (route grid placeholder)
- Dispatch actions (quick actions grid)
...
- **Shipments** (`#/list`) — a searchable, paginated, CSV-exportable table of 20 shipments.
- **Network reports** (`#/reports`) — two extra charts, three insight tiles and key takeaways.
