# Wardlight — Hospital Admin Dashboard Template

A calm boxed hospital administration dashboard template with bed-occupancy donut, appointments table with live filter and on-duty staff strip.
...
- Bed occupancy by ward (donut chart)
- Today’s appointments — data table with 7 realistic rows, status pills and row actions
- Ward updates — timestamped feed
- On-duty specialists (team avatar row)
...
- **Patients** (`#/list`) — a searchable, paginated, CSV-exportable table of 20 patients.
- **Operations reports** (`#/reports`) — two extra charts, three insight tiles and key takeaways.
