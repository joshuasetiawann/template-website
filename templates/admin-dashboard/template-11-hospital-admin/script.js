/* Wardlight — hospital admin dashboard interactions (vanilla JS, no dependencies) */
(function () {
  'use strict';
  var doc = document;
  var body = doc.body;
  ...
  /* ----- Live table filter ----- */
  var filterInput = qs('#tableFilter');
  if (filterInput) {
    var filterRows = qsa('#dataTable tbody tr');
    var emptyNote = qs('#emptyNote');
  ...
  /* ----- Chart data (Wardlight sample dataset) ----- */
  var WARDS = [{"label": "General medicine", "value": 142, "color": "#10B981"}, {"label": "ICU", "value": 18, "color": "#F0635C"}, {"label": "Maternity", "value": 38, "color": "#60A5FA"}, {"label": "Pediatrics", "value": 33, "color": "#E8A33D"}];
  renderDonut('wardDonut', WARDS);


  /* ----- Wardlight release upgrades: year, shimmer, reveal, pagination, export, tooltips ----- */
  /* ----- Reports view charts (rendered lazily on first show) ----- */
  window.__renderReports = function () {
    renderDonut('repChartA', [{"label": "Cardiology", "value": 28, "color": "var(--accent)"}, {"label": "General", "value": 24, "color": "var(--accent-2)"}, {"label": "Orthopaedics", "value": 18, "color": "#34D399"}, {"label": "ICU", "value": 12, "color": "#F87171"}, {"label": "Maternity", "value": 10, "color": "#FBBF24"}, {"label": "Paediatrics", "value": 8, "color": "#A78BFA"}]);
    renderDonut('repChartB', [{"label": "Low", "value": 96, "color": "var(--accent-2)"}, {"label": "Medium", "value": 74, "color": "#FBBF24"}, {"label": "High", "value": 38, "color": "#FB923C"}, {"label": "Critical", "value": 4, "color": "#F87171"}]);
  ...
  attachDonutTip(qs('#wardDonut'), function () { return wardsNow; }, function (s) { return s.value + ' beds'; });
  ...
  var WARDS_A = WARDS;
  var WARDS_B = [{"label": "General medicine", "value": 136, "color": "#10B981"}, {"label": "ICU", "value": 16, "color": "#F0635C"}, {"label": "Maternity", "value": 41, "color": "#60A5FA"}, {"label": "Pediatrics", "value": 29, "color": "#E8A33D"}];
  var WARD_LEG = { a: { lv: ["142 / 168", "18 / 20", "38 / 48", "33 / 48"], lp: ["61%", "8%", "16%", "15%"], c: "81%" },
                   b: { lv: ["136 / 168", "16 / 20", "41 / 48", "29 / 48"], lp: ["61%", "7%", "19%", "13%"], c: "78%" } };
  var wardsNow = WARDS;
  function applyRange(k) {
    wardsNow = k === 'b' ? WARDS_B : WARDS_A;
    renderDonut('wardDonut', wardsNow);
    var leg = WARD_LEG[k] || WARD_LEG.a;
    qsa('.donut-legend li').forEach(function (li, i) {
      var lv = qs('.lv', li), lp = qs('.lp', li);
      if (lv && leg.lv[i]) lv.textContent = leg.lv[i];
      if (lp && leg.lp[i]) lp.textContent = leg.lp[i];
    });
    var center = qs('.donut-center b');
    if (center) center.textContent = leg.c;
  }
