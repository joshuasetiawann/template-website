# Inkflow — Content Dashboard Template

A lavender editorial content dashboard template with pipeline bars, editorial mini-calendar, filterable posts table and publishing activity feed.

Part of a 100-template library of pure HTML/CSS/JS website templates. This template uses the boxed soft-pastel layout with very rounded cards.
...
- Animated horizontal bar chart — track fills sweep in from a JS data array
- Live table search filter (type to hide non-matching rows)
...
- Pipeline by stage (hbars chart)
- Posts pipeline — data table with 7 realistic rows, status pills and row actions
- Newsroom feed — timestamped feed
- Editorial calendar (mini calendar grid)
...
- **Posts** (`#/list`) — a searchable, paginated, CSV-exportable table of 20 posts.
- **Editorial reports** (`#/reports`) — two extra charts, three insight tiles and key takeaways.
