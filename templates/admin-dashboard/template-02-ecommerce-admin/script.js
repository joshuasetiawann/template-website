/* Cartloom — e-commerce admin dashboard interactions (vanilla JS, no dependencies) */
(function () {
  'use strict';
  var doc = document;
  var body = doc.body;
  ...
  /* ----- Chart data (Cartloom sample dataset) ----- */
  var SALES = [{"name": "Revenue", "color": "#7C3AED", "area": true, "dots": true, "data": [4820, 5210, 4640, 5980, 6420, 5760, 7210, 8427]}];
  var SALESLabels = ["May 29", "May 31", "Jun 2", "Jun 4", "Jun 6", "Jun 8", "Jun 10", "Jun 11"];
  function drawSALES(skip) {
    renderLine('salesLine', SALES, { labels: SALESLabels, skipAnim: skip, fmtY: function (v) { return '$' + (v >= 1000 ? (v / 1000).toFixed(1) + 'k' : v); } });
  }
  drawSALES(false);
  redraws.push(function () { drawSALES(true); });


  /* ----- Cartloom release upgrades: year, shimmer, reveal, pagination, export, tooltips ----- */
  /* ----- Reports view charts (rendered lazily on first show) ----- */
  window.__renderReports = function () {
    renderLine('repChartA', [{"name": "Revenue", "color": "var(--accent)", "area": true, "dots": true, "data": [42, 47, 44, 51, 49, 58, 62, 60, 66, 71, 69, 78]}], { labels: ["W1", "W2", "W3", "W4", "W5", "W6", "W7", "W8", "W9", "W10", "W11", "W12"], fmtY: function (v) { return '$' + v + 'k'; } });
    renderLine('repChartB', [{"name": "Sessions", "color": "var(--accent-2)", "area": true, "dots": true, "data": [820, 910, 870, 990, 1040, 1120, 1080]}, {"name": "Orders", "color": "var(--accent)", "dots": true, "data": [28, 33, 31, 38, 41, 47, 44]}], { labels: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"], fmtY: function (v) { return '' + v; } });
  ...
  var SALES_A = SALES, SALES_LA = SALESLabels;
  var SALES_B = [{"name": "Revenue", "color": "#7C3AED", "area": true, "dots": true, "data": [4310, 4690, 4150, 5320, 5660, 5020, 6280, 7140]}];
  var SALES_LB = ["May 15", "May 17", "May 19", "May 21", "May 23", "May 25", "May 27", "May 28"];
  function applyRange(k) {
    SALES = k === 'b' ? SALES_B : SALES_A;
    SALESLabels = k === 'b' ? SALES_LB : SALES_LA;
    drawSALES(false);
  }
