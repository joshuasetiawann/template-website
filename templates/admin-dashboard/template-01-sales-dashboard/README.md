# Northpeak — Sales Dashboard Template

A clean light sales admin dashboard template with revenue stats, an animated bar chart, deal pipeline table and activity feed.
...
- Range chips that re-render the main chart with a different dataset
...
- Revenue performance (bars chart)
- Recent deals — data table with 7 realistic rows, status pills and row actions
- Recent activity — timestamped feed
- Quick actions (quick actions grid)
- Top reps this month (top items list)
...
- **Deals** (`#/list`) — a searchable, paginated, CSV-exportable table of 20 deals.
- **Sales reports** (`#/reports`) — two extra charts, three insight tiles and key takeaways.
