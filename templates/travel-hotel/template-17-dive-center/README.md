# Abyssal Blue Dive Center — Dive Center Template

Abyssal Blue Dive Center is a deep, dark dive-centre template with a dive-day timeline, badge stats, a per-person booking widget and a dives-and-courses detail page.

## Features

- Layout: **V3 adventure dark — a dramatic dark hero, an expedition timeline, glassy badge stats and bold accents**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Dives, Trips & Courses") detail page
...
- Dives grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Drop into the blue.
- Choose your dive — six dives with prices
- The shape of a dive day (expedition timeline)
- Amenities — Why dive with us
...
- `packages.html` — the full "Dives, Trips & Courses" detail page
...
- **Dives:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
