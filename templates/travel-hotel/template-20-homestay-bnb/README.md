# Marigold House — Homestay B&B Template

Marigold House is a warm family homestay-and-B&B template with a host-family band, a how-it-works row, room cards and an our-rooms detail page.

## Features

- Layout: **V5 friendly local — warm tones, a host story band, sticker badges and a how-it-works step row**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Our Rooms & the Whole House") detail page
...
- Rooms grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Stay like family, not like a guest.
- Pick your room — six rooms with prices
...
- `packages.html` — the full "Our Rooms & the Whole House" detail page
...
- **Rooms:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
