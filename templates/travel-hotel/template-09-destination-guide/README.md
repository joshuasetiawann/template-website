# Lisbon Unfolded — Destination Guide Template

Lisbon Unfolded is an editorial, independent destination-guide template with neighbourhood itineraries, a day-plan detail page and ad-free, locally-written content.

## Features

- Layout: **V4 boutique editorial — oversized type, an asymmetric photo-tile collage and fine hairline rules**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Itineraries & Day Plans") detail page
...
- Itineraries grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Lisbon, unfolded by the people who live it.
- Ways to see the city — six itineraries with prices
- Amenities — Why this guide
...
- `packages.html` — the full "Itineraries & Day Plans" detail page
...
- **Itineraries:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
