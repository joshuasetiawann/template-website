# Casa Terramar — Villa Rental Template

Casa Terramar is a luxury private-villa-rental template with a full-bleed hero, exclusive whole-villa booking and a suite-by-suite detail page.

## Features

- Layout: **V1 luxury — full-bleed hero, a floating booking bar, serif headings, large room cards and hairline framing**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Suites & Whole-Villa Stays") detail page
...
- Suites grid of six options, each linking through to its full write-up on the packages page
...
- Hero — One villa. The whole headland.
- Inside Casa Terramar — six suites with prices
...
- `packages.html` — the full "Suites & Whole-Villa Stays" detail page
...
- **Suites:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
