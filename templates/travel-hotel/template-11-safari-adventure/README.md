# Acacia Plains Safaris — Safari Adventure Template

Acacia Plains Safaris is a dark, dramatic safari template with an expedition timeline, badge stats, a per-person enquiry widget and a safari-itineraries detail page.

## Features

- Layout: **V3 adventure dark — a dramatic dark hero, an expedition timeline, glassy badge stats and bold accents**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Safari Itineraries") detail page
...
- Safaris grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Track the wild where the road ends.
- Choose your safari — six safaris with prices
- The rhythm of a safari day (expedition timeline)
- Amenities — The safari difference
...
- `packages.html` — the full "Safari Itineraries" detail page
...
- **Safaris:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
