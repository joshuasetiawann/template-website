# Greenline Expeditions — Tour Operator Template

Greenline Expeditions is a fresh small-group tour-operator template with a per-person enquiry widget, six journeys and a day-by-day itineraries detail page.

## Features

- Layout: **V2 fresh modern — a split hero with the booking card alongside, an airy amenity icon grid and rounded surfaces**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Journeys & Itineraries") detail page
...
- Journeys grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Small groups. Wild places. Done right.
- Pick your expedition — six journeys with prices
- Amenities — Why travel with us
...
- `packages.html` — the full "Journeys & Itineraries" detail page
...
- **Journeys:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
