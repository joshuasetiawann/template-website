# Wildfern Glamping — Glamping Site Template

Wildfern Glamping is a friendly glamping-site template with a host story, a how-it-works row, bell-tent and safari-lodge cards and a canvas-stays detail page.

## Features

- Layout: **V5 friendly local — warm tones, a host story band, sticker badges and a how-it-works step row**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Canvas Stays & Pitches") detail page
...
- Stays grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Sleep under canvas, not compromise.
- Pick your pitch — six stays with prices
...
- `packages.html` — the full "Canvas Stays & Pitches" detail page
...
- **Stays:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
