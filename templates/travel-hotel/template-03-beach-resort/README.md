# Coralline Bay Resort — Beach Resort Template

Coralline Bay Resort is a fresh, airy beach-resort template with overwater villas, a split hero, a booking widget and a full villas-and-suites detail page.

## Features

- Layout: **V2 fresh modern — a split hero with the booking card alongside, an airy amenity icon grid and rounded surfaces**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Villas & Suites") detail page
...
- Villas grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Wake to the reef, not the alarm.
- Pick your patch of sand — six villas with prices
...
- `packages.html` — the full "Villas & Suites" detail page
...
- **Villas:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
