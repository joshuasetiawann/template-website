# Glacier Hollow Ski Resort — Ski Resort Template

Glacier Hollow Ski Resort is a fresh, modern ski-resort template with a split hero, slopeside lodge cards, a booking widget and a lodges-and-packages detail page.

## Features

- Layout: **V2 fresh modern — a split hero with the booking card alongside, an airy amenity icon grid and rounded surfaces**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Lodges & Ski Packages") detail page
...
- Lodges grid of six options, each linking through to its full write-up on the packages page
...
- Hero — First lifts, last light, door to the snow.
- Stay slopeside — six lodges with prices
...
- `packages.html` — the full "Lodges & Ski Packages" detail page
...
- **Lodges:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
