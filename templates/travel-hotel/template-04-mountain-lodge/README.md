# Blackpine Hollow Lodge — Mountain Lodge Template

Blackpine Hollow Lodge is a dark, adventurous mountain-lodge template with an expedition timeline, badge stats and a cabins-and-rooms detail page.

## Features

- Layout: **V3 adventure dark — a dramatic dark hero, an expedition timeline, glassy badge stats and bold accents**
- Multi-page: an `index.html` overview and a dedicated `packages.html` ("Cabins & Rooms") detail page
...
- Cabins grid of six options, each linking through to its full write-up on the packages page
...
- Hero — Where the trail ends and the fire begins.
- Bunk down at altitude — six cabins with prices
- The arc of a lodge day (expedition timeline)
...
- `packages.html` — the full "Cabins & Rooms" detail page
...
- **Cabins:** each card lives in the section marked `id="rooms"`; its "Details" link points at the matching `#pkg-n` block in `packages.html`.
