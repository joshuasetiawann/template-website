# Forge Grooming Lounge — Men's Grooming Lounge Template

A premium men's grooming lounge for cuts, shaves, skincare and a proper drink in hand.

A four-page, fully responsive men's grooming lounge website template built with pure HTML5, CSS3 and vanilla JavaScript — no frameworks, no build step, no external assets. Styled in the **dark-premium** variant.

## Features

- **Real multi-page site** — every navigation item leads to a genuine, fully-populated page.
- **Service menu & booking** — a complete, categorised treatment menu with durations and prices, plus a validated booking widget (service, date, time, groomer and contact) that shows a confirmation summary.
- **Packages, memberships & FAQ** — bundle pricing, an `#pricing` section and an accessible FAQ accordion.
- **Team, gallery, story & impact stats** — a full groomer line-up, CSS/SVG gallery tiles, salon story and an animated count-up statistics band.
