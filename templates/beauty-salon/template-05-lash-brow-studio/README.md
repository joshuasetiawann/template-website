# Wink & Arch — Lash & Brow Studio Template

A friendly lash and brow studio shaping eyes that wake up gorgeous, every single morning.

A four-page, fully responsive lash & brow studio website template built with pure HTML5, CSS3 and vanilla JavaScript — no frameworks, no build step, no external assets. Styled in the **modern-rounded** variant.

## Features

- **Real multi-page site** — every navigation item leads to a genuine, fully-populated page.
- **Service menu & booking** — a complete, categorised treatment menu with durations and prices, plus a validated booking widget (service, date, time, stylist and contact) that shows a confirmation summary.
- **Packages, memberships & FAQ** — bundle pricing, an `#pricing` section and an accessible FAQ accordion.
- **Team, gallery, story & impact stats** — a full stylist line-up, CSS/SVG gallery tiles, salon story and an animated count-up statistics band.
