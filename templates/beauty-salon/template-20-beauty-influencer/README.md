# Glow with Remi — Beauty Creator & Personal Brand Template

The personal brand of a beauty creator — tutorials, collaborations, masterclasses and one-to-one glow sessions.

A four-page, fully responsive beauty creator & personal brand website template built with pure HTML5, CSS3 and vanilla JavaScript — no frameworks, no build step, no external assets. Styled in the **playful-vibrant** variant.

## Features

- **Real multi-page site** — every navigation item leads to a genuine, fully-populated page.
- **Service menu & booking** — a complete, categorised treatment menu with durations and prices, plus a validated booking widget (service, date, time, creator and contact) that shows a confirmation summary.
- **Packages, memberships & FAQ** — bundle pricing, an `#pricing` section and an accessible FAQ accordion.
- **Team, gallery, story & impact stats** — a full creator line-up, CSS/SVG gallery tiles, salon story and an animated count-up statistics band.
