/* ============================================================
   BrightPath Tutoring -- Tutoring Center
   template-11-tutoring-center (V2)
   Vanilla JS: mobile nav, header scroll state, scrollspy, scroll-reveal, count-up stats, accordion, enroll validation, back-to-top, animated meters, read-more toggle
   ============================================================ */
(function () {
  'use strict';
  var d = document;
  ...
  var meters = qsa('[data-meter]');
  if (meters.length) {
    var fillMeter = function (m) {
      var bar = qs('.meter-fill', m);
      var val = m.getAttribute('data-meter') || '0';
      if (bar) { bar.style.width = val + '%'; }
      m.classList.add('filled');
    };
    if (hasIO && !reduceMotion) {
      var ioMeter = new window.IntersectionObserver(function (entries, obs) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) { fillMeter(entry.target); obs.unobserve(entry.target); }
        });
      }, { threshold: 0.5 });
      meters.forEach(function (m) { ioMeter.observe(m); });
    } else {
