/* ============================================================
   Sunnybrook Elementary -- Elementary School
   template-03-elementary-school (V3)
   Vanilla JS: mobile nav, header scroll state, scrollspy, scroll-reveal, count-up stats, accordion, enroll validation, back-to-top, fun-fact rotator, read-more toggle
   ============================================================ */
(function () {
  'use strict';
  var d = document;
  ...
  /* ---- fun fact rotator ---- */
  var factWrap = qs('[data-funfacts]');
  var factBtn = qs('[data-fact-btn]');
  if (factWrap && factBtn) {
    var facts = qsa('.fact', factWrap);
    var fIdx = 0;
    factBtn.addEventListener('click', function () {
      if (!facts.length) { return; }
      fIdx = (fIdx + 1) % facts.length;
      facts.forEach(function (f, j) { f.hidden = j !== fIdx; });
      factWrap.classList.remove('pop');
      void factWrap.offsetWidth;
      factWrap.classList.add('pop');
    });
  }
