/* ============================================================
   ForgeLabs Bootcamp -- Coding Bootcamp
   template-05-coding-bootcamp (V4)
   Vanilla JS: mobile nav, header scroll state, scrollspy, scroll-reveal, count-up stats, accordion, enroll validation, back-to-top, terminal typing, testimonial slider
   ============================================================ */
(function () {
  'use strict';
  var d = document;
  ...
  var term = qs('[data-terminal]');
  if (term) {
    var tLines = qsa('.t-line', term);
    if (tLines.length && !reduceMotion) {
      var texts = tLines.map(function (l) { return l.textContent; });
      tLines.forEach(function (l) { l.textContent = ''; l.classList.remove('done'); });
      var li = 0;
      var chi = 0;
      var typeStep = function () {
        if (li >= tLines.length) { term.classList.add('typed'); return; }
        var lineEl = tLines[li];
        var full = texts[li];
        if (chi <= full.length) {
          lineEl.textContent = full.slice(0, chi);
          chi += 1;
          window.setTimeout(typeStep, 26);
        } else {
