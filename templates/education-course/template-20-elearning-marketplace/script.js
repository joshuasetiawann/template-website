/* ============================================================
   Skillwave -- Online Course Marketplace
   template-20-elearning-marketplace (V2)
   Vanilla JS: mobile nav, header scroll state, scrollspy, scroll-reveal, count-up stats, accordion, enroll validation, back-to-top, chip filters, testimonial slider, read-more toggle
   ============================================================ */
(function () {
  'use strict';
  var d = document;
  ...
  /* ---- program chip filter ---- */
  var chipWrap = qs('[data-chips]');
  if (chipWrap) {
    var chips = qsa('.chip', chipWrap);
    var filterCards = qsa('[data-cat]');
    var searchBox = qs('[data-search]');
    var applyFilter = function () {
      var active = qs('.chip[aria-pressed="true"]', chipWrap);
      var cat = active ? active.getAttribute('data-chip') : 'all';
