/* Hammerstone Auctions — Buy Property at Auction in Manchester | property-site interactions (vanilla JS, dependency-free) */
(function () {
  "use strict";

  document.documentElement.classList.add("js");

  var BRAND = "Hammerstone Auctions";
  ...
  var cdEls = $all("[data-countdown]");
  if (cdEls.length) {
    var cdStart = Date.now();
    var renderCd = function (el, secs) {
      if (secs <= 0) {
        if (el.hasAttribute("data-cdgroup")) {
          $all("[data-cd]", el).forEach(function (b) { b.textContent = "0"; });
        } else {
      ...
      var sec = secs % 60;
      if (el.hasAttribute("data-cdgroup")) {
        var parts = { d: String(d), h: pad2(h), m: pad2(m), s: pad2(sec) };
        $all("[data-cd]", el).forEach(function (b) {
          var k = b.getAttribute("data-cd");
          if (parts[k] !== undefined) { b.textContent = parts[k]; }
        });
      } else {
        el.textContent = (d > 0 ? d + "d " : "") + pad2(h) + ":" + pad2(m) + ":" + pad2(sec);
      }
    };
    var cdTick = function () {
      var elapsed = Math.floor((Date.now() - cdStart) / 1000);
      cdEls.forEach(function (el) {
        var initial = parseInt(el.getAttribute("data-countdown") || "0", 10);
        renderCd(el, initial - elapsed);
      });
    };
    cdTick();
    window.setInterval(cdTick, 1000);
  }
