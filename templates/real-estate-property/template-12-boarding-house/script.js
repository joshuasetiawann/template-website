/* Kos Harmoni — Kos Nyaman & Strategis di Kemang, Jakarta | property-site interactions (vanilla JS, dependency-free) */
(function () {
  "use strict";

  document.documentElement.classList.add("js");

  var BRAND = "Kos Harmoni";
  ...
  if (enqForm) {
    ...
    enqForm.addEventListener("submit", function (e) {
      ...
      if (statusEl) {
        statusEl.textContent = "Terima kasih — pesan Anda sudah kami terima. Tim kami akan segera menghubungi.";
  ...
  if (calcBox) {
    ...
    var recalc = function () {
      ...
      var years = Math.min(Math.max(parseFloat(yearsIn.value) || 1, 1), 45);
      var rate = Math.min(Math.max(parseFloat(rateIn.value) || 0, 0), 30);
      ...
      if (yearsOut) { yearsOut.textContent = years + " yrs"; }
      if (monthlyEl) { monthlyEl.textContent = money(monthly) + "/bln"; }
      ...
      if (barEl && totalPaid > 0) {
        barEl.style.width = Math.max(6, Math.round(principal / totalPaid * 100)) + "%";
      }
    };
    ...
    window.dispatchInput = function (el) { if (el) { recalc(); } };
    recalc();
  }
