# Stackline — Tech Blog Template

Stackline is a multi-page tech blog template built as a classic editorial (split featured hero, three-column grid with a sticky right sidebar), with a full home page and a matching article reading view.
...
- Lead/hero post, a filterable grid of posts, a numbered trending list and a newsletter sign-up
...
- Syntax-styled code block in the article body, author bio, prev/next pager and three related cards on every article
...
- **Masthead / navigation** — brand mark plus jump links: Latest, Release radar, Masthead, Newsletter
- **Hero** — the featured post with category, byline, read time and calls to action
...
- **About the editor** — a short blurb introducing Priya Raghunathan
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (AI & ML, Web Platform, DevOps, Hardware, Open Source) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#2563EB`). Change them once to retheme both pages.
