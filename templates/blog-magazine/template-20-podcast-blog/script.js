/* ==========================================================================
   Static & Signal — Podcast Blog · script.js
   Template: template-20-podcast-blog (variant v5)
...
(function () {
  ...
  var SITE = { slug: 'template-20-podcast-blog', brand: 'Static & Signal', noun: 'posts', listen: true };
  ...
  if (newsForm) {
    ...
    newsForm.addEventListener('submit', function (e) {
      ...
      if (!value) {
        ...
        formMsg.textContent = 'Signal locked — The Flare transmits to your inbox Friday.';
  ...
  /* --- Fake episode player: play/pause toggle + EQ bars --- */
  var playButtons = qsa('[data-play]');
  if (playButtons.length) {
    playButtons.forEach(function (btn) {
      btn.addEventListener('click', function () {
        var episode = btn.closest('.episode');
        if (!episode) { return; }
        var wasPlaying = episode.classList.contains('is-playing');
        qsa('.episode.is-playing').forEach(function (other) {
          other.classList.remove('is-playing');
          var otherBtn = qs('[data-play]', other);
