# Static & Signal — Podcast Blog Template

Static & Signal is a multi-page podcast blog template built as a dark zine (dark theme, shelf/masonry layout, duotone covers, topic marquee), with a full home page and a matching article reading view.
...
- **Masthead / navigation** — brand mark plus jump links: Episodes, Show notes, Charts, Flare, Hosts
...
- **About the editor** — a short blurb introducing Ezra Quill & Mona Iwu
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Episodes, Show Notes, Behind the Mic, Guests, Gear) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#F59E0B`). Change them once to retheme both pages.
