# Little Wonders — Parenting Blog Template

Little Wonders is a multi-page parenting blog template built as a classic editorial (split featured hero, three-column grid with a sticky right sidebar), with a full home page and a matching article reading view.
...
- **Masthead / navigation** — brand mark plus jump links: Latest, Ages & stages, About us, Newsletter
...
- **About the editor** — a short blurb introducing Hannah Okoye
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Babies, Toddlers, Big Kids, Family Life, Real Talk) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#FF7059`). Change them once to retheme both pages.
