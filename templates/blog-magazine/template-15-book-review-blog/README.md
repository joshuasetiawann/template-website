# The Marginalia Review — Book Review Blog Template

The Marginalia Review is a multi-page book review blog template built as a minimal essay (centered reading column, large serif, list-style index, reading-progress bar), with a full home page and a matching article reading view.
...
- Lead/hero post, a filterable grid of reviews, a numbered trending list and a newsletter sign-up
...
- Star-rated verdict block with a read-it / skip-it breakdown, author bio, prev/next pager and three related cards on every article
...
- **Masthead / navigation** — brand mark plus jump links: Reviews, Staff shelf, The Flyleaf, About
- **Hero** — the featured review with category, byline, read time and calls to action
...
- **About the editor** — a short blurb introducing Eleanor Voss
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Fiction, Nonfiction, Poetry, Classics, In Translation) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#7D2A3A`). Change them once to retheme both pages.
