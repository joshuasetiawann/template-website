# Butter & Thyme — Recipe Blog Template

Butter & Thyme is a multi-page recipe blog template built as a classic editorial (split featured hero, three-column grid with a sticky right sidebar), with a full home page and a matching article reading view.
...
- Lead/hero post, a filterable grid of recipes, a numbered trending list and a newsletter sign-up
...
- Ingredients-and-method recipe card in the article body, author bio, prev/next pager and three related cards on every article
...
- **Masthead / navigation** — brand mark plus jump links: Recipes, Test kitchen, Meet the cook, Newsletter
- **Hero** — the featured recipe with category, byline, read time and calls to action
...
- **About the editor** — a short blurb introducing Marisol Vega
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Weeknight, Baking, Vegetarian, One Pot, Desserts) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#E8722C`). Change them once to retheme both pages.
