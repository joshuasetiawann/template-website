# Basis Point — Finance Blog Template

Basis Point is a multi-page finance blog template built as a classic editorial (split featured hero, three-column grid with a sticky right sidebar), with a full home page and a matching article reading view.
...
- Lead/hero post, a filterable grid of notes, a numbered trending list and a newsletter sign-up
...
- Stat-callout block with an inline sparkline, author bio, prev/next pager and three related cards on every article
...
- **Masthead / navigation** — brand mark plus jump links: Analysis, Market pulse, The desk, Memo
- **Hero** — the featured note with category, byline, read time and calls to action
...
- **About the editor** — a short blurb introducing Grace Okonjo
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Markets, Investing, Personal Finance, Fintech, Macro) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#0E9F6E`). Change them once to retheme both pages.
