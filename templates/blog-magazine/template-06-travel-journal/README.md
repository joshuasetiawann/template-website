# The Wanderlog — Travel Journal Template

The Wanderlog is a multi-page travel journal template built as a minimal essay (centered reading column, large serif, list-style index, reading-progress bar), with a full home page and a matching article reading view.
...
- Lead/hero post, a filterable grid of dispatches, a numbered trending list and a newsletter sign-up
...
- **Masthead / navigation** — brand mark plus jump links: Dispatches, Well-thumbed, About, The letter
- **Hero** — the featured dispatche with category, byline, read time and calls to action
...
- **About the editor** — a short blurb introducing Theo Marchetti
- **Newsletter** — an email sign-up with inline validation messages
- **Footer** — brand summary, quick links, topics (Field Notes, Itineraries, Slow Travel, Food Maps, Practicalities) and social icons
...
- **Colors & type** — every theme value lives in the `:root` custom properties at the top of `style.css` (accent is `#0E7E8A`). Change them once to retheme both pages.
