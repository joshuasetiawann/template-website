/* ==========================================================================
   Grindhouse Weekly — Movie Review Blog · script.js
   Template: template-07-movie-review-blog (variant v5)
...
(function () {
  ...
  var SITE = { slug: 'template-07-movie-review-blog', brand: 'Grindhouse Weekly', noun: 'reviews', listen: false };
  ...
  if (newsForm) {
    ...
    newsForm.addEventListener('submit', function (e) {
      ...
      if (!value) {
        ...
        formMsg.textContent = 'Ticket torn — The Midnight Reel screens in your inbox Friday.';
  ...
  /* --- Now-showing shelf: scroll controls --- */
  var shelf = qs('[data-shelf]');
  var shelfPrev = qs('[data-shelf-prev]');
  var shelfNext = qs('[data-shelf-next]');
  if (shelf && shelfPrev && shelfNext) {
    var shelfStep = function (dir) {
      shelf.scrollBy({ left: dir * 246, behavior: prefersReduced ? 'auto' : 'smooth' });
    };
    shelfPrev.addEventListener('click', function () { shelfStep(-1); });
    shelfNext.addEventListener('click', function () { shelfStep(1); });
  }
