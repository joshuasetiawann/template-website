# Hearthside Hardware & Home — Family-Run Hardware & Home Services Company Profile Template

Hearthside Hardware and Home is a warm amber local-business profile template with hand-crafted styling, priced services, neighbor testimonials and store hours.
...
- `index.html` — the complete one-page store profile (hero, since-1962 story, priced services, why-us, neighbor slider, certifications, family team, store gallery, contact form with hours)
- `services.html` — "Services & Stories" depth page: each homepage service as an anchored block (`#svc-1`…`#svc-6`) with what's included, a three-step how-it-works and the same board prices, three true neighbor stories with the dollars and days counted (`#case-1`–`#case-3`), an over-the-counter FAQ (its own `.svq` accordion bound in the appended script) and a CTA back to `index.html#contact`
