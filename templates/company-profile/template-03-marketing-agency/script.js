/* ==========================================================
   Studio Vermilion — Brand & Performance Marketing
   Template: template-03-marketing-agency (V3 Swiss minimal)
...
/* ---- v2: services depth page + demo-link guard (runs on both pages) ---- */
(function () {
  'use strict';

  /* placeholder socials (team rows, footer) carry data-demo-link and must never jump the page */
  document.addEventListener('click', function (e) {
    var demo = e.target && e.target.closest ? e.target.closest('[data-demo-link]') : null;
    if (demo) { e.preventDefault(); }
  });

  /* fold the depth-page furniture into the same reveal rhythm as the homepage */
  var still = false;
  try { still = !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches); } catch (mmErr) { still = false; }
  var depthEls = Array.prototype.slice.call(document.querySelectorAll('.sv-block, .sv-case, .sv-logo-row li'));
  if (!still && depthEls.length && 'IntersectionObserver' in window) {
    var svIo = new window.IntersectionObserver(function (entries, obs) {
      entries.forEach(function (entry) {
        if (!entry.isIntersecting) { return; }
        var el = entry.target;
        el.classList.add('in-view');
        window.setTimeout(function () { el.classList.remove('pre-reveal'); el.classList.remove('in-view'); }, 900);
        obs.unobserve(el);
      });
    }, { threshold: 0.08 });
    depthEls.forEach(function (el) { el.classList.add('pre-reveal'); svIo.observe(el); });
  }
})();
