/* ==========================================================
   Foxglove Estates — Residential & Investment Real Estate
   Template: template-10-real-estate-company (V1 corporate classic)
   Vanilla JS: mobile nav, validated contact form, count-up stats
   ========================================================== */
(function () {
  'use strict';
...
/* ---- v2: track-record page behaviours (page-safe on index & services) ---- */
(function () {
  'use strict';

  /* keep demo placeholder anchors ([data-demo-link]) from jumping the page */
  document.addEventListener('click', function (event) {
    var hit = event.target && event.target.closest ? event.target.closest('[data-demo-link]') : null;
    if (hit) { event.preventDefault(); }
  });

  /* fold-out questions on services.html use the namespaced .svq pattern */
  function toggleSvq() {
  ...
  var folds = document.querySelectorAll('.svq-btn');
  for (var i = 0; i < folds.length; i += 1) {
    folds[i].addEventListener('click', toggleSvq);
  }
})();
