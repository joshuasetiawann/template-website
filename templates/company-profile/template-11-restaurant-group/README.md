# Casa Fiorelli Group — Family Restaurant Group Company Profile Template

Casa Fiorelli Group is a warm restaurant-group profile template with menu-card services, a kitchen-process timeline and table-booking style contact.
...
- `index.html` — the one-page profile (hero, famiglia story, venues, kitchen timeline, testimonials, gallery, team, contact)
- `services.html` — "Tables & Stories" depth page: each room, the truck and both catering services opened up with menus, booking steps and per-head prices (`#svc-1`…`#svc-6`), three venue stories with covers and ratings (`#case-1`–`#case-3`), a booking FAQ and a CTA back to the host desk
...
- Gallery tiles on the homepage now open the matching story or venue page; demo placeholder links (team and footer socials) carry `data-demo-link` and never navigate
