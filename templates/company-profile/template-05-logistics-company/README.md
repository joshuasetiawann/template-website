# MeridianHaul Logistics — Freight, Warehousing & Last-Mile Company Profile Template

MeridianHaul is a logistics company profile template with route-line styling, process timeline, certification band and live-feeling stats.
...
- `index.html` — the one-page company profile (hero, about, services, load process, why-us, client wall, certifications, team, testimonials, contact)
- `services.html` — rate-card depth page: one block per service (`#svc-1`…`#svc-6` with inclusions, a "how a load moves" timeline and starting rates), three shipper case studies (`#case-1`…`#case-3`), a rate FAQ accordion and a CTA back to the quote form
...
Both pages cross-link through the nav and footer ("Services in Depth"); placeholder social links carry `data-demo-link` so they never navigate in the demo.
