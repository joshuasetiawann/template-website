# Solana Voyages — Boutique Travel & Tours Company Profile Template

Solana Voyages is an airy travel company profile template with boarding-pass styling, wave dividers, tabbed trip types and a filterable destination grid.
...
- `index.html` — the complete one-page travel profile (hero, about, tabbed trip styles, why-us, destination board, traveller mail, team, certifications, FAQ, contact form)
- `services.html` — "Trips & Stories" depth page: each homepage trip style as an anchored block (`#svc-1`…`#svc-6`) with what's included, a three-step planning flow and honest price hints, three real traveller stories with post-trip survey numbers (`#case-1`–`#case-3`), a pre-booking FAQ (driven by the same `.acc` accordion script) and a CTA back to `index.html#contact`
