/* ==========================================================
   IronRidge Constructions — Commercial Construction & Civil Works
   Template: template-02-construction-company (V2 modern overlay)
   Vanilla JS: mobile nav, validated contact form, service tabs, portfolio filter
   ========================================================== */
(function () {
  'use strict';
...
/* ---- v2: capabilities depth page — demo-link guard + spec-sheet accordion ---- */
(function () {
  'use strict';

  /* placeholder links (team mini-socials, footer socials) are inert by design */
  document.addEventListener('click', function (e) {
    var demo = e.target && e.target.closest ? e.target.closest('[data-demo-link]') : null;
    if (demo) { e.preventDefault(); }
  });

  /* owner FAQ on services.html: namespaced .svq accordion (this template ships
     no .acc-btn handler, so the binder lives here and stays null-safe) */
  var svqBtns = Array.prototype.slice.call(document.querySelectorAll('.svq-btn'));
  svqBtns.forEach(function (btn) {
    btn.addEventListener('click', function () {
