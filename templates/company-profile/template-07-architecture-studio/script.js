/* ==========================================================
   Atelier Northgrid — Architecture & Spatial Design
   Template: template-07-architecture-studio (V3 Swiss minimal)
   Vanilla JS: mobile nav, validated contact form, accordion
   ========================================================== */
(function () {
  'use strict';
...
/* ---- v2 sheet: depth-page behaviours shared by index.html and services.html ---- */
(function () {
  'use strict';

  /* demo placeholders: tm-social rows and footer socials hold data-demo-link; swallow the jump */
  document.addEventListener('click', function (ev) {
    var holder = ev.target && ev.target.closest ? ev.target.closest('[data-demo-link]') : null;
    if (holder) { ev.preventDefault(); }
  });

  /* let the discipline sheets and built studies join the reveal cadence, motion permitting */
  var calm = false;
  try { calm = !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches); } catch (e) { calm = false; }
  if (calm || !('IntersectionObserver' in window)) { return; }
  var sheets = Array.prototype.slice.call(document.querySelectorAll('.sv-block, .sv-case'));
  if (!sheets.length) { return; }
  var sheetIo = new window.IntersectionObserver(function (entries, obs) {
    entries.forEach(function (entry) {
      if (!entry.isIntersecting) { return; }
      var sheet = entry.target;
      sheet.classList.add('in-view');
      window.setTimeout(function () {
        sheet.classList.remove('pre-reveal');
        sheet.classList.remove('in-view');
      }, 900);
      obs.unobserve(sheet);
    });
  }, { threshold: 0.06 });
  sheets.forEach(function (sheet) {
    sheet.classList.add('pre-reveal');
    sheetIo.observe(sheet);
  });
})();
