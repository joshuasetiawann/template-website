# Nexcore Systems — Cloud & Software Engineering Company Profile Template

Nexcore Systems is a cloud and software engineering company profile template with count-up stats, service cards and a validated contact form.
...
- `index.html` — the one-page company profile (hero, about, services, why-us, CTA, team, client logo wall, testimonials, FAQ, certifications, contact)
- `services.html` — services & case-studies depth page: one block per service card (`#svc-1`…`#svc-6` with deliverables, 3-step process and starting prices), three client case studies (`#case-1`…`#case-3`), buying FAQ and a CTA back to the contact form
...
The nav and footer link both pages together ("Case Studies"); demo placeholder links (team and footer socials) carry `data-demo-link` and are guarded in JS so they never navigate.
