# Maison Ochre & Fen — Interior Design Studio Company Profile Template

Maison Ochre and Fen is a greige interior design profile template with framed gallery grids, serif elegance and an accordion of design services.
...
- `index.html` — the complete one-page studio profile (hero, philosophy, services accordion, studio manner, framed portfolio, certifications, team, client words, contact form)
- `services.html` — "Services & Projects" depth page: each design service as an anchored block (`#svc-1`…`#svc-6`) with deliverables, a three-step commission flow and an honest fee approach, three finished houses with the results they earned (`#case-1`–`#case-3`), a pre-commission FAQ (driven by the same `.acc` accordion script) and a CTA back to `index.html#contact`
