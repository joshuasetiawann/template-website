# Velvet Thyme — Event Catering Service Template

Velvet Thyme is an ivory-and-plum catering template with tasting menus, event packages, a quote request form and elegant service story.
...
- Layout: **V1 elegant dining — split hero, tasting-list menu typography, reservation split with hours card**
- Pure HTML5 / CSS3 / vanilla JavaScript — no frameworks, fonts, CDNs or build step; works from `file://`
- Tabbed menu with 12 dishes across 4 categories ($ pricing)
- Event quote form (name, email, event date, headcount) with inline error states and a success message (browser-side only)
...
- Per-template motif details layered over the shared base styles

## Sections

- Hero — Dinner for two hundred, plated like dinner for two.
- Our story — Lena Petros & Samuel Ash
- Menu (Canapés / Plated Dinners / Family-Style Feasts / Sweet Tables)
- Feature band
