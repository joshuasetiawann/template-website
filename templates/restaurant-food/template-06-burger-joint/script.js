/* ==========================================================
   Hatch & Griddle — Smash Burger Joint
   template-06-burger-joint (layout V5)
   Vanilla JS: mobile nav, menu chip filter, form validation,
   scrollspy, scroll-reveal, back-to-top, daily specials board
   ========================================================== */
(function () {
  'use strict';
  ...
  var dishCards = qsa('[data-menu-grid] .mi-card');
  var emptyNote = qs('[data-menu-empty]');
    ...
    chips.forEach(function (chip) {
      chip.addEventListener('click', function () {
        ...
        chips.forEach(function (cb) {
          var active = cb === chip;
          cb.classList.toggle('is-active', active);
          cb.setAttribute('aria-pressed', active ? 'true' : 'false');
        });
        var shown = 0;
        ...
        if (emptyNote) {
          if (shown === 0) { emptyNote.removeAttribute('hidden'); }
          else { emptyNote.setAttribute('hidden', ''); }
        }
      });
    });
  }
  ...
  /* ---------- chalkboard: rotate the daily special ---------- */
  var chalkRows = qsa('[data-specials] .chalk-row');
  var chalkOut = qs('[data-special-today]');
  var chalkPrev = qs('[data-special-prev]');
  var chalkNext = qs('[data-special-next]');
  if (chalkRows.length && chalkOut) {
    var chalkIdx = (new Date().getDay() + 6) % 7;
    var renderChalk = function () {
      chalkRows.forEach(function (row, i) {
        row.classList.toggle('is-today', i === chalkIdx);
      });
      var row = chalkRows[chalkIdx];
      var day = qs('.chalk-day', row);
      var name = qs('.chalk-meta strong', row);
      chalkOut.textContent = (day ? day.textContent : '') + ' \u2014 ' + (name ? name.textContent : '');
    };
    var stepChalk = function (dir) {
      chalkIdx = (chalkIdx + dir + chalkRows.length) % chalkRows.length;
      renderChalk();
    };
    if (chalkPrev) { chalkPrev.addEventListener('click', function () { stepChalk(-1); }); }
    if (chalkNext) { chalkNext.addEventListener('click', function () { stepChalk(1); }); }
    renderChalk();
  }
