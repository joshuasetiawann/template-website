# Rohan Mehta — Software Engineer Portfolio Template

A pure HTML/CSS/JS portfolio template for a software engineer, styled as a playful bento-grid layout with stat cards and tag-filtered projects.
...
- JavaScript interactions:
  ...
  - Tag-based project filtering with pressed-state filter chips
...
- Avatar hero card
- Bento grid: about, count-up stats, toolstack, top skills
- Tag-filtered project cards (6 projects)
- Experience timeline
- Career highlights
...
- `--radius` — global corner radius (currently 10px)
- `--font-head`, `--font-body` — system font stacks for headings and body text
- `--pad-section`, `--speed` — section rhythm and transition speed

Dark-mode colors are overridden under the `.theme-dark` class further down the file.
