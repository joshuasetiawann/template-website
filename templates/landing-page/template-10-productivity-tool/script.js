/* Plannery — Productivity Tool | landing page interactions (vanilla JS, no dependencies) */
...
(function () {
  ...
  if (spySections.length && "IntersectionObserver" in window) {
    var spyObs = new IntersectionObserver(function (entries) {
      each(entries, function (entry) {
        each(spySections, function (pair) {
          if (pair.sec === entry.target && entry.isIntersecting) {
            each(spyLinks, function (l) { l.classList.remove("is-current"); l.removeAttribute("aria-current"); });
            pair.link.classList.add("is-current");
            pair.link.setAttribute("aria-current", "true");
          }
        });
      });
    }, { rootMargin: "-40% 0px -55% 0px" });
    each(spySections, function (pair) { spyObs.observe(pair.sec); });
  }
  ...
  if (rlAnnounce) {
    var rlAnnKey = "plannery-announce-dismissed";
    var rlAnnSaved = null;
