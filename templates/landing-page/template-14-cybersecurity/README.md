# Cipherline — Cybersecurity Landing Page Template

A complete, dependency-free landing page template for Cipherline, a fictional cybersecurity brand, styled as a dark gradient layout with a glow hero and bento feature grid.
...
- `index.html` — the Cipherline landing page (all sections in-page).
...
- `--bg` (#0C1117) and `--surface` / `--surface-2` — page and card backgrounds
- `--text` (#E6EDF3) and `--muted` — type colors
- `--accent` (#3FB950) and `--accent-2` (#2EA043) — brand colors used by buttons, links and gradients
