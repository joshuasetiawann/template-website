# Brightreel — Creator Tool Landing Page Template

A complete, dependency-free landing page template for Brightreel, a fictional creator tool brand, styled as a bold geometric layout with angled dividers and shape decorations.
...
- `index.html` — the Brightreel landing page (all sections in-page).
...
- `--bg` (#F6F1FE) and `--surface` / `--surface-2` — page and card backgrounds
- `--text` (#2B2150) and `--muted` — type colors
- `--accent` (#8B5CF6) and `--accent-2` (#FFD43B) — brand colors used by buttons, links and gradients
