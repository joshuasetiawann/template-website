/* Nova Summit — Event Conference | landing page interactions (vanilla JS, no dependencies) */
...
(function () {
  ...
  var billButtons = document.querySelectorAll(".bill-btn");
  var billNote = document.querySelector(".bill-note");
  if (billButtons.length) {
    each(billButtons, function (btn) {
      btn.addEventListener("click", function () {
        var period = btn.getAttribute("data-bill");
        if (!period) { return; }
        each(billButtons, function (b) {
          var on = b === btn;
          b.classList.toggle("is-active", on);
          b.setAttribute("aria-pressed", on ? "true" : "false");
        });
        each(document.querySelectorAll(".amount"), function (amt) {
          var val = amt.getAttribute("data-" + period);
          if (val) { amt.textContent = val; }
        });
        if (billNote) {
          var note = billNote.getAttribute("data-" + period);
          if (note) { billNote.textContent = note; }
        }
      });
    });
  }
  ...
  if (rlAnnounce) {
    var rlAnnKey = "novasummit-announce-dismissed";
    var rlAnnSaved = null;
