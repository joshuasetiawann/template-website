# Nova Summit — Event Conference Landing Page Template

A complete, dependency-free landing page template for Nova Summit, a fictional event conference brand, styled as a bold geometric layout with angled dividers and shape decorations.
...
- `index.html` — the Nova Summit landing page (all sections in-page).
...
- `--bg` (#221330) and `--surface` / `--surface-2` — page and card backgrounds
- `--text` (#FBF4FF) and `--muted` — type colors
- `--accent` (#FF6B6B) and `--accent-2` (#FFB347) — brand colors used by buttons, links and gradients
