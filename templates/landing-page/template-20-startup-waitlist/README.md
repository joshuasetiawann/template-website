# Meridian — Startup Waitlist Landing Page Template

A complete, dependency-free landing page template for Meridian, a fictional startup waitlist brand, styled as an editorial, typography-led narrative layout.
...
- Live waitlist counter that increments after a successful signup
...
- `index.html` — the Meridian landing page (all sections in-page).
...
- `--text` (#0A0A0A) and `--muted` — type colors
- `--accent` (#0A0A0A) and `--accent-2` (#6E6E6E) — brand colors used by buttons, links and gradients
- `--accent-soft`, `--border`, `--shadow` — tints, hairlines and elevation
- `--radius` (0px) / `--radius-lg` — corner rounding across all components
- `--container` (1100px) — page max-width
