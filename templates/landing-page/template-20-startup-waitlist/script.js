/* Meridian — Startup Waitlist | landing page interactions (vanilla JS, no dependencies) */
...
(function () {
  ...
  var SITE = {
    brand: "Meridian",
    successMsg: "You are in line. Check your inbox for your position and referral link.",
    successBtn: "In line ✓",
  ...
  /* ----- Waitlist counter bump ----- */
  function bumpCounter() {
    var counter = document.getElementById("waitCount");
    if (!counter) { return; }
    var current = parseInt(counter.textContent.replace(/[^0-9]/g, ""), 10);
    if (isNaN(current)) { return; }
    counter.textContent = formatNum(current + 1, 0);
  }
  ...
  var emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
  each(document.querySelectorAll("form[data-validate]"), function (form) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      input.classList.remove("input-error");
      msg.textContent = SITE.successMsg;
      msg.className = "form-msg success";
      input.value = "";
      if (button) {
        button.textContent = SITE.successBtn;
        button.disabled = true;
        setTimeout(function () {
          button.textContent = SITE.submitLabel;
