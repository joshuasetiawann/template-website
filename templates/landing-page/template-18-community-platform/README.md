# Glowroom — Community Platform Landing Page Template

A complete, dependency-free landing page template for Glowroom, a fictional community platform brand, styled as a dark gradient layout with a glow hero and bento feature grid.
...
- `index.html` — the Glowroom landing page (all sections in-page).
...
- `--bg` (#150F1E) and `--surface` / `--surface-2` — page and card backgrounds
- `--text` (#FAF3FF) and `--muted` — type colors
- `--accent` (#EC4899) and `--accent-2` (#FB923C) — brand colors used by buttons, links and gradients
- `--accent-soft`, `--border`, `--shadow` — tints, hairlines and elevation
- `--radius` (18px) / `--radius-lg` — corner rounding across all components
