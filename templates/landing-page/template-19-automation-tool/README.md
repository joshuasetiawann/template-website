# Loopforge — Automation Tool Landing Page Template

A complete, dependency-free landing page template for Loopforge, a fictional automation tool brand, styled as a dark gradient layout with a glow hero and bento feature grid.
...
- `index.html` — the Loopforge landing page (all sections in-page).
...
- `--bg` (#0F1420) and `--surface` / `--surface-2` — page and card backgrounds
- `--text` (#ECF1FA) and `--muted` — type colors
- `--accent` (#F97316) and `--accent-2` (#FDBA74) — brand colors used by buttons, links and gradients
