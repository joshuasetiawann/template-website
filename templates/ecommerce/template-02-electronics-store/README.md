# Nordvolt — Electronics Store Template

Nordvolt builds obsessively tested audio, charge and desk tech — shop headphones, hubs and keyboards engineered in Oslo.

## Features

- Layout variant: **immersive dark** — pure HTML5/CSS3/vanilla JS, no dependencies
...
- Sticky filter toolbar with a JS sort dropdown (closes on outside click and Esc)
- Featured slider with previous/next controls and a slide counter
- Product cards with hover overlay quick actions
...
- `--accent` (#3B82F6), `--accent-2` (#22D3EE), `--accent-ink` — brand colors
- `--radius`, `--radius-sm` — corner rounding (currently 12px)
