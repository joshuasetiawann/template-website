/* Nebula Vault — Digital Product Store | vanilla JS, no dependencies */
(function () {
  'use strict';
  var CONFIG = { brand: 'Nebula Vault', currency: '$', freeShip: 100, addedNote: 'added to your cart', emptyNote: 'Your cart is empty — the vault is open.', bagWord: 'cart', newsletterNote: 'Subscribed — Thursday drops will hit your inbox first.', checkoutNote: 'Demo vault — checkout disabled', shipUnlocked: 'Bonus gradient pack unlocked — added to your downloads.', shipPrefix: 'Add ', shipSuffix: ' more to unlock the bonus gradient pack.' };
  ...
  var itemsEl = qs('[data-drawer-items]');
  var totalEl = qs('[data-drawer-total]');
  var countEl = qs('[data-drawer-count]');
  var shipEl = qs('[data-drawer-ship]');
  var shipFill = qs('[data-ship-fill]');
  var items = [];
  function openDrawer() {
    if (!drawer) { return; }
  ...
  function renderCart() {
    ...
    items.forEach(function (item, i) {
      ...
      li.innerHTML = '<span class="cart-thumb"' + (item.grad ? ' style="background:' + item.grad + '"' : '') + '></span>' +
        '<div><p class="cl-name">' + item.name + '</p><p class="cl-price">' + money(item.price) + '</p>' +
        '<div class="qty-row"><button class="qty-btn" type="button" data-dec="' + i + '" aria-label="Decrease quantity">&minus;</button>' +
        '<span class="qty-val">' + item.qty + '</span>' +
        '<button class="qty-btn" type="button" data-inc="' + i + '" aria-label="Increase quantity">+</button></div></div>' +
        '<div class="cl-side"><span class="cl-sum">' + money(item.price * item.qty) + '</span>' +
        '<button class="line-remove" type="button" data-remove="' + i + '">Remove</button></div>';
      itemsEl.appendChild(li);
    });
    if (totalEl) { totalEl.textContent = money(total); }
    if (countEl) { countEl.textContent = '(' + qty + ')'; }
    if (drawer) { drawer.classList.toggle('has-items', items.length > 0); }
    if (shipEl) {
      if (total >= CONFIG.freeShip) {
        shipEl.textContent = CONFIG.shipUnlocked;
        shipEl.classList.add('free');
      } else {
        shipEl.textContent = CONFIG.shipPrefix + money(CONFIG.freeShip - total) + CONFIG.shipSuffix;
        shipEl.classList.remove('free');
      }
    }
    if (shipFill && CONFIG.freeShip > 0) {
      shipFill.style.width = Math.min(100, Math.round((total / CONFIG.freeShip) * 100)) + '%';
    }
    setBadge(qty);
  }
  function addToCart(name, price, grad) {
    var found = null;
    items.forEach(function (it) { if (it.name === name) { found = it; } });
    if (found) { found.qty += 1; } else { items.push({ name: name, price: price, qty: 1, grad: grad || '' }); }
    renderCart();
    openDrawer();
  }
  if (itemsEl) {
    itemsEl.addEventListener('click', function (e) {
      ...
      if (!btn) { return; }
      if (btn.hasAttribute('data-remove')) { items.splice(parseInt(btn.getAttribute('data-remove'), 10), 1); }
      if (btn.hasAttribute('data-inc')) { items[parseInt(btn.getAttribute('data-inc'), 10)].qty += 1; }
      if (btn.hasAttribute('data-dec')) {
        var i = parseInt(btn.getAttribute('data-dec'), 10);
        items[i].qty -= 1;
  ...
  var checkoutBtn = qs('[data-checkout]');
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', function () {
      var old = checkoutBtn.textContent;
      checkoutBtn.textContent = CONFIG.checkoutNote;
      checkoutBtn.disabled = true;
      setTimeout(function () { checkoutBtn.textContent = old; checkoutBtn.disabled = false; }, 1700);
    });
  }
  ...
  /* ----- sort dropdown (closes on outside click / Esc) ----- */
  var sortWrap = qs('[data-sort]');
  if (sortWrap) {
    var sortBtn = qs('[data-sort-btn]', sortWrap);
    var sortMenu = qs('[data-sort-menu]', sortWrap);
    var sortLabel = qs('[data-sort-label]', sortWrap);
    var gridEl = qs('[data-grid]');
    var sortOpen = false;
    var setSortOpen = function (v) {
      sortOpen = v;
      if (sortMenu) { sortMenu.classList.toggle('open', v); }
      if (sortBtn) { sortBtn.setAttribute('aria-expanded', v ? 'true' : 'false'); }
    };
    if (gridEl) { qsa('[data-cat]', gridEl).forEach(function (card, i) { card.setAttribute('data-i', String(i)); }); }
    var sortGrid = function (mode) {
      if (!gridEl) { return; }
      var list = qsa('[data-cat]', gridEl);
      ...
      list.forEach(function (card) { gridEl.appendChild(card); });
    };
    if (sortBtn) { sortBtn.addEventListener('click', function (e) { e.stopPropagation(); setSortOpen(!sortOpen); }); }
    document.addEventListener('click', function (e) {
      if (sortOpen && !sortWrap.contains(e.target)) { setSortOpen(false); }
    });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && sortOpen) { setSortOpen(false); if (sortBtn) { sortBtn.focus(); } }
    });
    qsa('[data-sort-value]', sortWrap).forEach(function (opt) {
      opt.addEventListener('click', function () {
        qsa('[data-sort-value]', sortWrap).forEach(function (o) { o.classList.remove('is-active'); });
        opt.classList.add('is-active');
        if (sortLabel) { sortLabel.textContent = opt.textContent; }
        sortGrid(opt.getAttribute('data-sort-value'));
        setSortOpen(false);
      });
    });
  }
  ...
  if (slider) {
    ...
    if (prevBtn) { prevBtn.addEventListener('click', function () { goTo(slideIndex - 1); }); }
    if (nextBtn) { nextBtn.addEventListener('click', function () { goTo(slideIndex + 1); }); }
  }
