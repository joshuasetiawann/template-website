# Eunoia Skin — Skincare Store Template

Eunoia Skin makes barrier-first skincare with proven percentages — ten essentials, refillable glass, zero filler.

## Features

- Layout variant: **classic boutique** — pure HTML5/CSS3/vanilla JS, no dependencies
- Add-to-cart with animated badge counter and rich toast confirmations (product name + "View bag" action)
- Animated cart count badge on every add-to-cart action
- Category filter pills that live-filter the product grid
- Split hero with a layered gradient product visual
...
- 8 products with concept-specific names, prices and gradient/SVG artwork (no image files)
...
Add-to-cart works identically across all five pages (same badge and toast confirmation), and the navigation "Shop" link plus the home product cards link the pages together.
...
- `--accent` (#5E8B6F), `--accent-2` (#25332B), `--accent-ink` — brand colors
- `--radius`, `--radius-sm` — corner rounding (currently 14px)
- `--font-display`, `--font-body` — system font stacks
- `--container` — page max-width (1140px)
