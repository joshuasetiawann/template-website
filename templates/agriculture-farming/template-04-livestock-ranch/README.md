# Iron Ridge Ranch — Livestock Ranch Template

A rugged livestock-ranch template with a pasture-raised meat shop, bulk-box ordering and a four-generation ranch story.

## Features

- Full meat catalogue with a JS category filter, sort and live "Showing N" count
- Reserve a beef box block plus an ordering &amp; pricing section with a sample price list
...
- Variant V1 (rustic-warm) design language with a unique palette, type and cow motif
...
- index.html — homepage (hero, about teaser, meat preview, practices, gallery, stats, testimonials, CTA)
- products.html — full meat grid with filter chips, bulk meat share block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Meat, Our farm, Practices, Gallery, Team and Visit.
...
- Meat data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
