# Goldacre Grains — Heritage Grain Farm Template

A wholesome heritage-grain template with a stone-milled flour shop, bakers' wholesale ordering and a field-to-loaf story.

## Features

- Full grain catalogue with a JS category filter, sort and live "Showing N" count
- Open a bakers' account block plus an ordering &amp; pricing section with a sample price list
...
- Variant V1 (rustic-warm) design language with a unique palette, type and wheat motif
...
- index.html — homepage (hero, about teaser, grain preview, practices, gallery, stats, testimonials, CTA)
- products.html — full grain grid with filter chips, monthly flour standing order block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Grain, Our farm, Practices, Gallery, Team and Visit.
...
- Grain data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
