# Mycelia Gardens — Gourmet Mushroom Farm Template

A friendly gourmet-mushroom template with a fresh-mushroom shop, grow-kit range and an indoor-cultivation story.

## Features

- Full mushrooms catalogue with a JS category filter, sort and live "Showing N" count
- Get a mushroom box block plus an ordering &amp; pricing section with a sample price list
...
- Variant V5 (wholesome-market) design language with a unique palette, type and mushroom motif
...
- index.html — homepage (hero, about teaser, mushrooms preview, practices, gallery, stats, testimonials, CTA)
- products.html — full mushrooms grid with filter chips, weekly chef's box block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Mushrooms, Our farm, Practices, Gallery, Team and Visit.
...
- Mushrooms data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
