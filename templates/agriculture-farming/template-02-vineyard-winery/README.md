# Stonecrest Vineyards — Vineyard & Winery Template

A refined estate-winery template with a wine collection, tasting reservations and a vineyard heritage story.

## Features

- Full wines catalogue with a JS category filter, sort and live "Showing N" count
- Join the wine club block plus an ordering &amp; pricing section with a sample price list
...
- Variant V3 (premium-estate) design language with a unique palette, type and grape motif
...
- index.html — homepage (hero, about teaser, wines preview, practices, gallery, stats, testimonials, CTA)
- products.html — full wines grid with filter chips, seasonal allocation block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Wines, Our farm, Practices, Gallery, Team and Visit.
...
- Wines data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
