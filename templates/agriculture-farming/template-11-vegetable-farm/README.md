# Furrow & Field — Market Vegetable Farm Template

A hearty market-garden template with a seasonal veg shop, weekly veg-box CSA and a no-dig, soil-first story.

## Features

- Full vegetables catalogue with a JS category filter, sort and live "Showing N" count
- Join the veg box block plus an ordering &amp; pricing section with a sample price list
...
- Variant V1 (rustic-warm) design language with a unique palette, type and carrot motif
...
- index.html — homepage (hero, about teaser, vegetables preview, practices, gallery, stats, testimonials, CTA)
- products.html — full vegetables grid with filter chips, weekly veg share block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Vegetables, Our farm, Practices, Gallery, Team and Visit.
...
- Vegetables data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
