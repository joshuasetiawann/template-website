# Trueseed Company — Heirloom Seed Supplier Template

A friendly heirloom-seed catalog template with a sortable seed shop, grow guides and a seed-saving heritage story.

## Features

- Full seeds catalogue with a JS category filter, sort and live "Showing N" count
- Save with a seed club block plus an ordering &amp; pricing section with a sample price list
...
- Variant V5 (wholesome-market) design language with a unique palette, type and seed motif
...
- index.html — homepage (hero, about teaser, seeds preview, practices, gallery, stats, testimonials, CTA)
- products.html — full seeds grid with filter chips, seasonal seed selection block, ordering &amp; pricing, CTA
...
2. Every navigation item leads to a real, fully built page: Seeds, Our farm, Practices, Gallery, Team and Visit.
...
- Seeds data lives inline in `products.html`; each card carries `data-cat` and `data-price` for filtering and sorting.
